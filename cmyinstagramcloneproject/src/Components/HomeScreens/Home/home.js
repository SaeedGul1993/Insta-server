                ...
                return (
                    <Card key={index} className="home-card">
                        <div style={{ display: "flex", justifyContent: 'space-between' }}>
                            <h2><Link to={item.postedBy._id !== props.userInfo._id ? `/profile/${item.postedBy._id}` : '/profile'}>
                                <span>
                                    <img className="avatar-style1" src={item.postedBy.pic} />
                                </span>
                                {item.postedBy.name}</Link></h2> {item.postedBy._id === props.userInfo._id && <DeleteIcon onClick={() => deletePost(item._id)} className="delete-btn" />}
